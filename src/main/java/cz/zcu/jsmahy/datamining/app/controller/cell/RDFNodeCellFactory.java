...
public class RDFNodeCellFactory extends ListCell<RDFNode> {
	...
	public RDFNodeCellFactory(final ListView<RDFNode> rdfList) {
		...
		editItem.textProperty()
		        .bind(Bindings.format("Add line for \"%s\"", textProperty()));
		...
		deleteItem.textProperty()
		          .bind(Bindings.format("Delete \"%s\"", textProperty()));
	...
	public static String formatRDFNode(RDFNode node) {
		...
		if (node.isResource()) {
			...
			LOGGER.debug("Resource \"{}\"", localName);
			return localName;
		}
