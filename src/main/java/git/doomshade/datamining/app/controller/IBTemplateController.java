...
public class IBTemplateController implements Initializable {
    ...
    private void setupTree() {
        ...
        treeTableViewRemove.setOnMouseClicked((e) -> {
            list.remove(tree.getSelectionModel().selectedItemProperty().get().getValue());
        });
    ...
    private static final class BooleanValidator extends ValidatorBase {
        ...
        protected void eval() {
            if (srcControl.get() instanceof TextInputControl) {
                ...
                if (Arrays.stream(TRUE_VALUES).anyMatch(x -> x.equalsIgnoreCase(text))) {
                    hasErrors.set(false);
                } else if (Arrays.stream(FALSE_VALUES).anyMatch(x -> x.equalsIgnoreCase(text))) {
                    hasErrors.set(false);
                } else {
                    hasErrors.set(true);
                }
    ...
    private static final class BooleanTextFieldEditorBuilder implements EditorNodeBuilder<Boolean> {
        ...
        public Boolean getValue() {
            return Arrays.stream(BooleanValidator.TRUE_VALUES).anyMatch(x -> x.equalsIgnoreCase(textField.getText()));
        }
