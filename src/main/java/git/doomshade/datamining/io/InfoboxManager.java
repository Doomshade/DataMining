...
import static git.doomshade.datamining.io.IOValidator.validateFileName;
...
public final class InfoboxManager {
    ...
    public static void downloadTemplates(final String fileName) throws IllegalArgumentException {
        ...
        s.setOnFailed(x -> {
            ...
            L.throwing(x.getSource().getException());
        });
    ...
    public static InfoboxTemplate createInfoboxTemplate(final String template) throws IllegalArgumentException {
        ...
        for (int i = 1; i < map.length; i++) {
            ...
            if (kvm.groupCount() >= 2 && kvm.group(2).toLowerCase().contains("required")) {
                required.add(key);
            } else {
                optional.add(key);
            }
        ...
        protected final Task<Void> createTask() {
            return new Task<>() {
                ...
                protected Void call() throws Exception {
                    ...
                    final Document doc = Jsoup.connect(INFOBOX_URL).get();
                    ...
                    for (final Element el : templates) {
                        ...
                        final Document page = Jsoup.connect(pageURL).get();

                        // the infobox data is in the first "<pre>"
                        final Element usage = page.select("pre").first();
                        if (usage == null) {
                            continue;
                        }
